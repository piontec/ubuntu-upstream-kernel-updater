    ...
    def __init__(self, prefix, *, convert_charrefs=True):
        ...
        if prefix is None:
            ...
            if groups[1] is None:
                ...
                self.ver_regexp = re.compile(f'^v{groups[0]}' + r'\.' + groups[1][1:] + r'(\.\d+)?/$')
...
class SingleVerHTMLParser(HTMLParser):
    def __init__(self, arch, flavor, version, *, convert_charrefs=True):
        ...
        self.file_name_regexp = re.compile(r"linux.*" + version + r".*((_all\.deb)|(" + flavor + ".*_" + arch +
                                           r"\.deb))")
...
run_res = subprocess.run(["shasum", "-c", "CHECKSUMS"], capture_output=True, cwd=dirpath)
